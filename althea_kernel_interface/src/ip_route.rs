...
/// Internal unit testing function for get_default_route()
fn get_default_route_internal(stdout: String) -> Result<Option<DefaultRoute>, Error> {
...
pub fn get_default_route() -> Result<Option<DefaultRoute>, Error> {
    let output = run_command("ip", &["route", "list", "default"])?;

    let stdout = String::from_utf8(output.stdout).unwrap();
    get_default_route_internal(stdout)
}

pub fn set_route(to: &IpRoute) -> Result<(), Error> {
    let to = to.to_string();
    let to: Vec<&str> = to.split_whitespace().collect();
    let mut def_route = vec!["route", "add"];
    def_route.extend(to);
    run_command("ip", &def_route)?;
    Ok(())
}
...
pub fn update_settings_route(
    ...
    let def_route = match get_default_route()? {
        Some(route) => route,
...
pub fn manual_peers_route(
    ...
    let changed = update_settings_route(settings_default_route)?;
    match settings_default_route {
        Some(d) => {
            set_route(&IpRoute::ToSubnet(ToSubnet {
                dst: *endpoint_ip,
                subnet: 32,
                via: Some(d.via),
                nic: d.nic.to_string(),
...
pub fn restore_default_route(
    ...
    let current_route = get_default_route()?;
        ...
        Some(d) => {
            if d.is_althea_default_route() {
                ...
                if let Some(route) = settings_default_route {
                    set_route(&IpRoute::DefaultRoute(route.clone()))?;
                }
            } else {
...
fn test_get_default_route_invalid() {
    let route = "1.2.3.4/16 dev interface scope link metric 1000";
    let res = get_default_route_internal(route.to_string());
...
192.168.36.0/24 dev vmnet8 proto kernel scope link src 192.168.36.1";
    ...
    let result = get_default_route_internal(stdout.to_string())
...
fn test_set_route() {
    ...
    assert_eq!(
        route.to_string(),
        "127.0.0.1  via 127.0.0.2  dev eno3p proto static "
    );
    let route = IpRoute::ToSubnet(ToSubnet {
        dst: "127.0.0.1".parse().unwrap(),
        subnet: 32,
    ...
    assert_eq!(route.to_string(), "127.0.0.1/24  dev eno3p proto static ");
    ...
    assert_eq!(
        route.to_string(),
        "127.0.0.1/24  via 127.0.0.2  dev eno3p proto static  src 127.0.0.2 "
    );
}
...
fn test_set_default_route() {
    ...
    assert_eq!(
        correct.to_string(),
        "default via 192.168.8.1 dev wifiinterface proto static"
    );
    ...
    assert_eq!(
        correct.to_string(),
        "default via 192.168.8.1 dev wifiinterface proto static src 127.0.0.2"
    );
}
