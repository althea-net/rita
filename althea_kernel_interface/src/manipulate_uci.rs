...
impl KernelInterface {
    ...
    pub fn set_uci_var(&mut self, key: &str, value: &str) -> Result<bool,Error> {
        ...
        if !output.stderr.is_empty() {
            return Err(Error::RuntimeError(format!("recieved error while setting UCI: {}", String::from_utf8(output.stderr)?)))
        }
        Ok(true)
    }
    ...
    pub fn get_uci_var(&mut self, key: &str) -> Result<String,Error> {
        ...
        if !output.stderr.is_empty() {
            return Err(Error::RuntimeError(format!("recieved error while getting UCI: {}", String::from_utf8(output.stderr)?)))
        }
    ...
    pub fn uci_commit(&mut self) -> Result<bool,Error> {
        ...
        if !output.stderr.is_empty() {
            return Err(Error::RuntimeError(format!("recieved error while commiting UCI: {}", String::from_utf8(output.stderr)?)))
        }
        Ok(true)
    }
}
