...
export ROUTER_IP=heartbeat-test-client
...
ssh -L localhost:4877:localhost:4877 root@$ROUTER_IP NO_REMOTE_LOG=TRUE RUST_BACKTRACE=FULL RUST_LOG=TRACE /tmp/$RITA_VERSION --config=/etc/$RITA_VERSION.toml &> out2.log
