...
pub async fn tick_payment_controller() {
    ...
    // this may be too clever, what we are doing is creating a series of
    // futures describing the operations we want to perform, then running
    // them all in parallel, in the ideal world this helps a very active node
    // send tons of payments and retries at once. But there is a higher
    // chance of implementation errors and other strange race conditions.
    // don't be afraid to remove this and just await in the loop
    let mut payment_futures = Vec::new();
    let mut retry_futures = Vec::new();
    for pmt in outgoing_payments {
        let fut = make_payment(pmt);
    ...
    let _ = join_all(payment_futures).await;
