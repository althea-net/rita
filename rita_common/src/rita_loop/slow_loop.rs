...
const HAP_MAX_15_MIN_LOAD_AVG: f32 = 4.0;
...
/// If the load average for a hap router is over 4 for the last 15 minutes its CPU is not fast enough to
/// handle the amount of traffic it needs to, so we reboot as a way to reset it.
fn check_15min_load_avg_haps() {
    let model = get_rita_client().network.device;
    let hw_info = get_hardware_info(model.clone());
    match (model, hw_info) {
        (None, _) => error!("Model name not found?"),
        (Some(mdl), Ok(info)) => {
            if mdl.contains("mikrotik_hap-ac2")
                && info.load_avg_fifteen_minute > HAP_MAX_15_MIN_LOAD_AVG
            {
                info!("15 minute load average too high, rebooting!");
