...
use althea_types::{EthAddress, Identity, PaymentTx, LocalIdentity};
...
use rita_common::payment_controller::{PaymentController, MakePayment};
    ...
    fn handle(&mut self, _: Tick, ctx: &mut Context<Self>) -> Self::Result {
        if let Some(ref id) = self.exit_id {
            ...
            if r.status() == StatusCode::Ok {
                ...
                ctx.notify_later(Tick, Duration::from_secs(5));
                Err(ExitManagerError::ExitRequestError(
                    String::from(format!(
